...
export const CartItemComponent = ({item}) => {
  ...
  return (
    <View className="flex-row h-[6rem] shadow-sm rounded-lg justify-between  p-2 mx-4 bg-white">
        <Image source={{uri: item.product.image}} className="h-full aspect-square" resizeMode="contain" />
        ...
        <View className="items-start justify-center h-full ml-3">
          ...
          <Text className="text-blue-500 font-bold">{item.product.price}$ <Text className="text-black font-normal">Size: {item.size}</Text> </Text>
        </View>
